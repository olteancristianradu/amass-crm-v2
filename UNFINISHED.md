...
| P2-PROCKPIT-FE | Build FE widget UI for Pro Cockpit (selectable + drag-drop layout). | Backend feed endpoint shipped `7fd93f6`; UI is the next user-visible surface. | open | next session | Approach: TanStack Query polls `/api/v1/cockpit/feed` every 60s; renders widgets from a per-user layout in localStorage initially; promote to `cockpit_layouts` Prisma table when multi-device persistence is requested. |
| P2-IMPORTER-EXCEL | Activate Excel adapter (`xlsx` package + tests). | Romanian SMBs export from Excel; this is the highest-value next adapter after CSV. | open | next session | Activation plan documented in `apps/api/src/modules/importer/adapters/excel.adapter.ts`. Bundle cost ~500KB; test with 10k-row sheet for memory ceiling. |
...
| P2-IMPORTER-PDF | Activate PDF adapter with text + OCR fallback. | Hardest adapter; high-value for invoice imports. | open | next session(s) | Three flavors: text PDF (pdf-parse), scanned PDF (Claude vision), fillable form (pdf-lib). Recommend starting with text PDF only. |
| P2-BRANCH-PROTECT | Enable branch protection on main with status checks required. | Prevents agents/humans from pushing CI-failing code into main. | open | needs GitHub UI access or REST API call | Settings → Branches → Add rule → main → Require status checks (CI, CodeQL, Secret Scan) → Block force push → Require linear history. Don't enable "Require PR review" — solo dev would lock self out. |
