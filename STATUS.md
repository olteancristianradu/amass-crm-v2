...
| CedarGuard policy coverage | **50/64 controllere** au `@RequireCedar` (sesiune 2026-04-28: 18 → 50 în trei batch-uri, 109 handlere). Cele 14 rămase sunt categorial unprotectable: scaffold-501 cu return 501 (`scim`, `webauthn`, `sync`), public/pre-auth (`auth`, `totp`, `portal`, `sso`, `health`), webhook callbacks semnate de IdP (`calls-webhook`, `email-tracking`), pure-read tabular (`audit`, `notifications`, `reports`, `pipelines`). |
