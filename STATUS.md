...
| CedarGuard policy coverage | **48/64 controllere** au `@RequireCedar` (sesiune 2026-04-28: 18 → 48 în două batch-uri, 105 handlere noi). Cele 16 rămase sunt scaffold-501 (`scim`/`webauthn`/`sync`/`push`), public/auth (`auth`/`totp`/`portal`/`sso`), webhook callbacks (`calls-webhook`/`email-tracking`) sau citire-doar (`audit`/`reports`/`pipelines`/`notifications`/`ai`/`exports`-listings/`webhooks`/`forecasting`/`approvals`-listings). |
