...
8. **Tests alongside code.** Vitest unit + integration (testcontainers + real Postgres). **Current measured reality (from `pnpm --filter @amass/api vitest run --config vitest.config.unit.ts --coverage` on main, 2026-05-15):** 58.23% line, 78.78% branch, 65.25% function across the api package (1145 passing tests in 115 spec files). **Target: ≥80% on security-critical services (`auth`, `billing`, `calls`, `invoices`, `deals`, `audit`, `prisma`)** as a prerequisite to v1 launch; other modules ramp to ≥50% before GA. **Critical-7 status (2026-05-15, with DI/DTO exclusions in vitest.config.unit.ts):** auth 96.2% ✓✓, billing 95.8% ✓✓, invoices 91.9% ✓, audit 89.7% ✓, deals 88.5% ✓, calls 87.4% ✓, prisma 86.4% ✓. **All 7 clear 80% line; auth + billing pass 95%.** Coverage excludes `*.module.ts`, `*.dto.ts`, `dto.ts`, `src/main.ts`, prisma generated client — Nest DI boilerplate without runtime logic. Overall api: 64.5% line (was 58.2%). Next: ramp non-critical modules toward 50% before GA. Don't claim "tested" for a module that lacks a `.spec.ts`. Re-measure with `pnpm --filter @amass/api vitest run --config vitest.config.unit.ts --coverage` before declaring progress.
