...
8. **Tests alongside code.** Vitest unit + integration (testcontainers + real Postgres). **Current measured reality (from `pnpm --filter @amass/api vitest run --config vitest.config.unit.ts` on main, 2026-05-16):** 1414+ unit tests passing in 140+ spec files (web: 117 unit tests). **Target: ≥80% on security-critical services (`auth`, `billing`, `calls`, `invoices`, `deals`, `audit`, `prisma`)** as a prerequisite to v1 launch; other modules ramp to ≥50% before GA. **Critical-7 status (2026-05-15):** auth 96.2% ✓✓, billing 95.8% ✓✓, invoices 91.9% ✓, audit 89.7% ✓, deals 88.5% ✓, calls 87.4% ✓, prisma 86.4% ✓. **All 7 clear 80% line; auth + billing pass 95%.** Coverage excludes `*.module.ts`, `*.dto.ts`, `dto.ts`, `src/main.ts`, prisma generated client — Nest DI boilerplate without runtime logic. **B-batch features (16 PRs total, complete 2026-05-16):** B1 Sync 6/6 (gateway + publishers + FE consumer + presence + offline buffer + load-test scaffold), B2 WebAuthn 5/5 (Passkey schema + register + login + FE × 2 + backup-codes lifecycle), B3 SCIM 5/5 (Users + Groups + bearer auth + Okta E2E + meta endpoints). Don't claim "tested" for a module that lacks a `.spec.ts`. Re-measure with `pnpm --filter @amass/api vitest run --config vitest.config.unit.ts --coverage` before declaring progress.
