 ...
 *   SMOKE_TENANT_SLUG=amass   # optional, only needed if the email has multiple tenants
 *
...
test.describe('auth smoke', () => {
  ...
  test('user can log in and reach the dashboard', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /conectare|log ?in|autentific/i }).click();

    const tenantPicker = page.getByRole('heading', { name: /alege spațiul de lucru/i });
    if (await tenantPicker.isVisible({ timeout: 3_000 }).catch(() => false)) {
      expect(slug, 'SMOKE_TENANT_SLUG is required when the account belongs to multiple tenants').toBeTruthy();
      await page.locator('label').filter({ hasText: slug! }).getByRole('radio').check();
      await page.getByRole('button', { name: /continuă|continue/i }).click();
    }

    // Either we land directly on the dashboard, or the router redirects /app → /dashboard.
    await expect(page).toHaveURL(/\/app(\/welcome)?\/?$/i, { timeout: 10_000 });
