...
|---|---:|---|---|---|
| SEC-006 | P1 | AI worker manual `/process/call` can fetch arbitrary recording URLs after static bearer auth. | `25f096b` (`fix(security): close audit gaps and harden integrations`, 2026-05-03) | `apps/ai-worker/app/main.py` now requires `Authorization: Bearer ${AI_WORKER_SECRET}` and fail-fasts with 503 if `AI_WORKER_SECRET` is empty (`if not expected: raise HTTPException(503)`). Verified locally `[verificat 2026-05-05]`: source at `apps/ai-worker/app/main.py:104-117`. SSRF residual: `pipeline.py` still fetches caller-supplied `recordingUrl` with redirects — host allow-list/private-IP block hardening **not yet applied**, tracked separately. |
...
| SEC-007 | P2 | `WEBHOOK_TRUSTED_HOSTS` was a production SSRF footgun: dev escape hatch left set in prod would bypass DNS/IP rebind check. | `<pending-commit-after-this-edit>` (2026-05-05) | `apps/api/src/config/env.ts` `prodOnlyChecks` now rejects any non-empty `WEBHOOK_TRUSTED_HOSTS` when `NODE_ENV=production`. Regression in `apps/api/src/config/env.prod-checks.spec.ts` (4 tests). Local: full API suite 980/980 pass. |
